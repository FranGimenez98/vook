...
export default function UpdateVook({ open, onClose, vookData, refreshData }) {
  return (
    <Dialog
      open={open}
      onClose={onClose}
    ...
    >
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <Dialog.Panel className="w-full max-w-lg max-h-[30rem] rounded-lg bg-white p-2 flex items-center flex-col">
          <Dialog.Title>Edit Vook</Dialog.Title>
          <Formik
            initialValues={{
              title: vookData?.title,
              description: vookData?.description,
            }}
            onSubmit={async (values) => {
              try {
                const res = await axios.put("/api/vooks", {
                  id: vookData.id,
                  title: values.title,
